            ...
            <TableBody>
              {paginatedDemandes.map((demandeconge, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>
                    {Array.isArray(demandeconge.planifications) && demandeconge.planifications.some((planification) => planification?.statut !== "ANNULE") ? (
                      getPlanificationBadge(demandeconge, canReadPlanification)
                    ) : (
                      <div className="space-y-2">
                        <Badge variant="outline" className="whitespace-nowrap">
                          Non planifie
                        </Badge>
                        {canCreatePlanification && canCreatePlanificationForDemande(demandeconge) ? (
                          <Button asChild size="sm" variant="outline">
                            <Link href={`/dashboard/planification?prefillDemandeCongeId=${demandeconge.id}`}>
                              Creer planification
                            </Link>
                          </Button>
                        ) : null}
              ...
              {paginatedDemandes.length === 0 && (
                <TableRow>
                  <TableCell colSpan={canConfirmDemandes ? 7 : 6} className="py-4 text-center text-muted-foreground">
                    Aucun resultat
                  </TableCell>
                </TableRow>
              )}
