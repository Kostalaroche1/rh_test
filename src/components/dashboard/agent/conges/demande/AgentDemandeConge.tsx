...
export default function AgentDemandeConge() {
  ...
  const canReadDemandes = hasAnyPermission(auth, ["conge.read", "conge.request", "conge.update", "conge.delete"]);
  const canCreateDemande = hasAnyPermission(auth, ["conge.request", "conge.create"]);
  const canManageOwnDemandes = hasAnyPermission(auth, ["conge.update", "conge.delete"]);
                  ...
                  <TableBody>
                    {paginatedDemandes.map((item) => (
                      <TableRow key={item.id} className="hover:bg-muted/50">
                        ...
                        <TableCell>{formaDate(item.dateDebut)}</TableCell>
                        <TableCell>{formaDate(item.dateFin)}</TableCell>
                        <TableCell>{item.typeConge?.libelle}</TableCell>
                        <TableCell>
                          <Badge className={getStatutBadgeColor(item.statut)}>{item.statut}</Badge>
                        </TableCell>
                        {canManageOwnDemandes && (
                          <TableCell className="flex items-center justify-end gap-2">
                            <Button
                              variant="outline"
                              size="sm"
                              disabled={getStatutValue(item.statut, "agent")}
              ...
              <form className="flex flex-col gap-4" onSubmit={updateDemande}>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <Input id="editDateDebut" type="date" value={formaDate(demande.dateDebut)} onChange={(e) => setDemande((prev) => ({ ...prev, dateDebut: e.target.value }))} required />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="editDateFin">Date de fin</Label>
                    <Input id="editDateFin" type="date" value={formaDate(demande.dateFin)} onChange={(e) => setDemande((prev) => ({ ...prev, dateFin: e.target.value }))} required />
                  </div>
                </div>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="editDateDemande">Date de la demande</Label>
                    <Input id="editDateDemande" type="date" value={formaDate(demande.dateDemande)} onChange={(e) => setDemande((prev) => ({ ...prev, dateDemande: e.target.value }))} required />
                  </div>
                  <div className="flex flex-col gap-1.5">
                    <Label>Type de conge</Label>
