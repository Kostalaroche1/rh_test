...
{/* Chart des présences */}
      <Card className="w-full min-h-[400px] border border-border bg-card py-4 shadow-sm">
        ...
        <TabsContent value="conges" className="flex flex-col gap-6">
          <Button variant="outline" onClick={() => setOpenDialog(true)} className="w-full justify-start">
            <PlusCircle className="w-5 h-5 mr-2" /> Nouvelle demande
          </Button>
            ...
            <CardContent className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {demandes.map((demande, idx) => (
                    <TableRow key={idx} className="hover:bg-muted/50">
                      <TableCell>{demande.start}</TableCell>
                      <TableCell>{demande.end}</TableCell>
